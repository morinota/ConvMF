...
from model.matrix_factorization import MatrixFactrization, RatingLog
from model.model_cnn_nlp import CnnNlpModel
...
class ConvMF(object):
    def __init__(
        ...
        self.mf = mf
        if self.mf is None:
            self.mf = MatrixFactrization(ratings, n_factor, user_lambda, item_lambda, n_item)

        # model architecture
        self.convolution = CnnNlpModel()
    ...
    def predict(self, users: List[int], items: List[int]) -> List[np.ndarray]:
        # item factor Vを取得
        item_factors = self.convolution(x=np.array([self.item_descriptions[i] for i in items]))
        # user factor Uを所得
    ...
    def update_latent_factor(self, n_trial=3):
        ...
        self.mf.fit(n_trial=n_trial, document_vectors=self._embedding())
